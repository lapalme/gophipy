...
def processFile(fileName,regex=None,trace=False,saveInOut=False):
    ...
    for line in open(fileName,encoding='utf-8'):
        ...
        if len(line)==0: ## traiter l'AMR
            ...
            if regex==None or re.search(regex,amrStr,re.DOTALL): 
                no+=1
                ...
                if out==None:
                    print("@@@:Bad realization from:amrStr")
                else:
                    print(out)
                    if saveInOut:
                        if re.search("Erroneous realization from jsRealB expression",out)!=None:
                            sntOut.write("*** jsRealB error ***\n")
                        else:
...
def createExcelWB(orig,generated,gophiPyBleu,spring,springBleu):    
    wb = Workbook()
    ws = wb.active
    c(ws,1,1,"AMR")
    c(ws,1,2,"base gen")
...
def showAMRsFile(fileName,regex=r"",trace=False,createExcel=False):
    ...
    if saveInOut:
        ...
        if createExcel:
            orig=open(resultDir+"sntIn.txt",encoding="utf-8")
            generated=open(resultDir+"sntOut.txt",encoding="utf-8")
            spring=open("/Users/lapalme/Dropbox/AMR/amr_annotation_3.0/data/amrs/split/test/amr-release-3.0-amrs-test-sample-25-5-SPRING.out",encoding="utf-8")
            bleuSpring=34.62
            wb=createExcelWB(orig,generated,bleuGophiPy,spring,bleuSpring) 
